...
EXCLUDED_SOURCE_TERMS = {
    "Snake Plant": ["rattlesnake plant", "calathea lancifolia"],
}
...
def _contains_phrase(text: str, phrase: str) -> bool:
    """Return true only when the normalized phrase appears on word boundaries."""
    return re.search(rf"\b{re.escape(_normalize(phrase))}\b", text) is not None
...
def _match_plant(folder: Path) -> str | None:
    ...
    for plant_name, aliases in PLANT_ALIASES.items():
        excluded_terms = EXCLUDED_SOURCE_TERMS.get(plant_name, [])
        if any(_contains_phrase(combined_text, term) for term in excluded_terms):
            continue

        for alias in aliases:
            if _contains_phrase(combined_text, alias):
                return plant_name
    return None
